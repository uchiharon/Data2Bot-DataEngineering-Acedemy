...
def word_giver(player1,player2):
   result = choice([player1,player2])
   return result
...
def game_code():
    ...
    print("""
          ...
          -------------------------
    """)
    player_name_2  = get_player_name()

    word_giver_name = word_giver(player_name_1,player_name_2)
    time.sleep(2)
    fruit = getpass.getpass(prompt =f"\n\n\n\n\n\n\n\n\n\n\n{word_giver_name}, give a fruit name:\n->")

    if word_giver_name == player_name_1:
       guesser = player_name_2
    else:
       guesser = player_name_1

    print(f"{guesser} you will be the one guessing\nThe hang-man begins")
    ...
    while no_of_wrong < 6 and no_of_correct < fruit_charaters:
        ...
        if guess in fruit:
            ...
            no_of_correct = len(correct)
            print(correct_guest_display(fruit, no_of_correct, hangman_image, fruit_charaters, no_of_wrong, guesser))
        else:
            wrong.append(guess)
            no_of_wrong = len(wrong)
            print(wrong_guest_display(fruit, no_of_wrong, hangman_image, guesser))
